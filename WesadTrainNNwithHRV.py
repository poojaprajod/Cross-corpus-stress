...
for s in subs:
    ...
    if not trained:
        ...
        callbacks = [  # EarlyStopping(monitor='val_acc', patience=10, min_delta=0.0001, mode='max'),
            ModelCheckpoint(filepath=f'nn_model_ecg_hrv_val_sub{s}.h5', monitor='val_loss', save_best_only=True)
        ]
    ...
    model_full.load_weights(f'nn_model_ecg_hrv_val_sub{s}.h5')
...
for s in subs:
    loaded_model.load_weights(f'nn_model_ecg_hrv_val_sub{s}.h5')
    preds = loaded_model.predict(X_swell_hrv)
    state = np.round(preds)

    print(classification_report(ys_swell.astype(int), state))
