...
for s in subs:
    ...
    if not trained:
        ...
        callbacks = [  # EarlyStopping(monitor='val_acc', patience=10, min_delta=0.0001, mode='max'),
            ModelCheckpoint(filepath=f'best_model_ecg_val{s}_znorm_deepecg.h5',
        ...
        with open("model_ecg_deepecg.json", "w") as json_file:
            json_file.write(model_json)

    model_full.load_weights(f'best_model_ecg_val{s}_znorm_deepecg.h5')
